...
func assertComplexitySessionLogContext(
	...
	t.Helper()
	require.Equal(t, complexitySessionHash(state.Key), ctx.Value(schemas.BifrostContextKeyGovernanceComplexitySessionID))
	assert.NotEqual(t, state.ID, ctx.Value(schemas.BifrostContextKeyGovernanceComplexitySessionID), "raw session identity reached log context")
